...
caprate_treas <- function(){
  ...
  offrate <- naWindow(4*100*1000*offm/(offq+debt_off)) - treas.qtr # NOI data in millions, market cap data in thousands
  indrate <- naWindow(4*100*1000*indm/(indq+debt_ind)) - treas.qtr # Multiply by 100 to get cap rates not as decimals
  retrate <- naWindow(4*100*1000*retm/(retq+debt_ret)) - treas.qtr
  aptrate <- naWindow(4*100*1000*aptm/(aptq+debt_apt)) - treas.qtr
  lodrate <- naWindow(4*100*1000*lodm/(lodq+debt_lod)) - treas.qtr
  allrate <- naWindow(4*100*1000*allm/(allq+debt_all)) - treas.qtr
  
  tislist <- list(allrate, offrate, indrate, retrate, aptrate, lodrate)
  
  rplot.line(tislist,
             Title = "Spread of Smoothed Capitalization Rate\nto Treasury Yield",
